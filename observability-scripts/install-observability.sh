...
cp "$SCRIPT_DIR/0k-rag-log-error.sh" "$TARGET_DIR/.claude/scripts/"
cp "$SCRIPT_DIR/0k-rag-health-check.sh" "$TARGET_DIR/.claude/scripts/"
cp "$SCRIPT_DIR/0k-rag-token-report.sh" "$TARGET_DIR/.claude/scripts/"
cp "$SCRIPT_DIR/0k-rag-latency-report.sh" "$TARGET_DIR/.claude/scripts/"
cp "$SCRIPT_DIR/0k-rag-error-report.sh" "$TARGET_DIR/.claude/scripts/"
...
echo "  ${GREEN}./.claude/scripts/0k-rag-health-check.sh --period=24h${NC}"
echo "    └─ System health dashboard (24h/7d/30d)"
echo ""
echo "  ${GREEN}./.claude/scripts/0k-rag-token-report.sh 2026-01${NC}"
echo "    └─ Monthly token usage and cost report"
echo ""
echo "  ${GREEN}./.claude/scripts/0k-rag-latency-report.sh 2026-01${NC}"
echo "    └─ Monthly latency performance report"
echo ""
echo "  ${GREEN}./.claude/scripts/0k-rag-error-report.sh 2026-01${NC}"
...
echo "  3. Check health dashboard: ./.claude/scripts/0k-rag-health-check.sh"
echo ""
